...
// GET  /api/posts        — list all posts (admin sees unpublished; public list omits them)
// POST /api/posts        — create a post (admin only)
// Query: ?published=1 limits to published.
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    return requireAuth(async (req, res) => {
      ...
      try {
        // Auto-create cloaked affiliate links from product URLs.
          ...
          if (Array.isArray(normalizedProducts)) {
            for (let i = 0; i < normalizedProducts.length; i++) {
              ...
              if (prod.link_url) {
                ...
                prod.link_url = undefined; // don't store raw URL in products_json
              }
            }
          }
        }
